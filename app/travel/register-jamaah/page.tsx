...
export default function AddJamaahPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/web/travel", {
      ...
      .catch(() => {
        // bisa tambah error handling jika perlu
      });
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!idTravelAgent) {
      setNotif({
        message: "Travel Agent belum tersedia, coba refresh halaman.",
        type: "error",
      });
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        setNotif({
          message: `❌ Gagal simpan: ${result.error || "Terjadi kesalahan"}`,
          type: "error",
        });
      }
    } catch (error) {
    ...
    <div className="p-6 min-h-screen flex bg-gray-50">
      ...
      <div className="w-full max-w-2xl bg-white p-10 rounded-2xl shadow-lg">
        <h2 className="text-xl font-semibold text-center mb-6 text-gray-800">
          Import Data Jamaah (Excel)
        </h2>

        <a
          href="/template/template-mabrurX.xlsx"
          download
          className="block w-full text-center bg-blue-100 hover:bg-blue-200 text-blue-700 font-medium py-3 rounded-xl transition mb-4"
        >
        ...
        <button
          className="mt-4 w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-3 rounded-xl transition"
          onClick={() => {
            /* proses upload nanti di sini */
          }}
        >
          Upload Excel
        </button>
